...
class Param(object):
    ...
    def checkSavingPath(self):

        fileInPath = f"{self.sPath}_exp.txt"

        if os.path.exists(fileInPath):

            print(f"{self.saveName} already exists in this directory. Saving file as:")

            path, name = os.path.split(fileInPath)
            root = '_'.join(name.split('_')[0:-1])
            inds = [i for i, c in enumerate(root) if c.isdigit()] 
            
            if len(inds)>0 and inds[-1]==len(root)-1:
                c_ = -1
                while np.diff(root[c_:]==1):
                    c_ -= 1
                    
                counter = int(root[inds[c_]:]) # get filiname rep#
                root = root[0:inds[c_]]
            else:
                counter = 0

            while os.path.exists(fileInPath):

                counter +=1
                fileInPath = f"{path}/{root}{counter}_exp.txt"

            self.saveName = f"{root}{counter}"
            self.sPath = f"{path}/{root}{counter}"
            
            #print(f"{self.saveName}")
            
            self.nameEd = True # was filename edited
        else:
            self.nameEd = False
         
        self.logParam(fileInPath, mode='a')
